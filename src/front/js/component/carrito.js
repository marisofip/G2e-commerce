  ...
  return (
    <div className="carShopping">
      <button
        ...
        onClick={() => actions.setMostrarCarShop()}
      >
        
              <i
                className="fa-solid fa-cart-shopping fa-2x text-body"
                key={index}
              /> <p> {carro.cantidad}</p>
        ...
        {store.carShopping.map((el, index) => {
          return (
            ...
            <li key={index} className="list-group-item">
              ...
              <li
                onClick={() => actions.eliminarCarShop(index)}
                className="far fa-trash-alt ms-1 p-1"
              />
            </li>
            // </div>
          );
