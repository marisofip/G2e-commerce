...
def create_product():
    ...
    if not precio:
        return jsonify({"message": "Precio is required"}), 400
    if img:
    ...
    if resp_img:
        product.img = resp_img['secure_url']
...
def create_categroriat():
    ...
    if foundCategoria:
        return jsonify({"message": "Categoria already exists"}), 400
    ...
    if resp_img:
        categoria.img = resp_img['secure_url']
